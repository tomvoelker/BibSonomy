...
public class OldPersonNameUtils {
	 ...
	 * @return the result
	 */
	public static List<OldPersonName> extractList(final String authorField) {
		final List<OldPersonName> authors = new LinkedList<OldPersonName>();
		if (present(authorField)) {
			final Scanner t = new Scanner(authorField);
			t.useDelimiter(PERSON_NAME_DELIMITER);
			while (t.hasNext()) {
				final OldPersonName a = new OldPersonName();
				a.setName(t.next());
				authors.add(a);
			}
		}
	...
	public static String lastFirstToFirstLast(final String name) {
		if (present(name)) {
			final int indexOf = name.indexOf(OldPersonName.LAST_FIRST_DELIMITER);
			if (indexOf >= 0) {
	...
	public static String lastFirstToFirstLastMany(final String names) {
		if (present(names)) {
			if (names.contains(OldPersonName.LAST_FIRST_DELIMITER)) {
	 ...
	 * @param personName 
	 */
	public static void discoverFirstAndLastName(final String name, final OldPersonName personName) {
		if (present(name)) {
			/*
			 ...
			 */
			final int indexOfComma = cleanedName.indexOf(OldPersonName.LAST_FIRST_DELIMITER);
			if (indexOfComma >= 0) {
				/*
	...
	public static String getFirstPersonsLastName(final String person) {
		if (present(person)) {
			 ...
			 */
			final OldPersonName personName = new OldPersonName();
	...
	public static String normalizePersonNames(final String names, final boolean lastFirstNames) {
		...
		final List<OldPersonName> personNames = new LinkedList<OldPersonName>();
		for (final String name : names.split(PERSON_NAME_DELIMITER)) {
			personNames.add(new OldPersonName(name));
		}
	 ...
	 */
	public static String serializePersonNames(final List<OldPersonName> personNames, final boolean lastFirstNames) {
		if (!present(personNames)) return null;
		final StringBuilder sb = new StringBuilder();
		int i = personNames.size();
		for (final OldPersonName personName : personNames) {
			i--;
	 ...
	 */
	public static String serializePersonName(final OldPersonName personName, final boolean lastFirstName) {
		...
		if (lastFirstName) {
			 ...
			 delim = OldPersonName.LAST_FIRST_DELIMITER + " ";
		} else {
