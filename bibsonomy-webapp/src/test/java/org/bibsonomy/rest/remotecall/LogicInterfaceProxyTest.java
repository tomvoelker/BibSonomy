...
public class LogicInterfaceProxyTest implements LogicInterface {
	...
	public String createUser(User user) {
		EasyMock.expect(serverLogic.createUser(PropertyEqualityArgumentMatcher.eq(user, "apiKey", "IPAddress", "basket", "gender", "interests", "hobbies", "profession", "openURL", "place", "spammer", "settings"))).andReturn(user.getName() + "-new");
		EasyMock.replay(serverLogic);
	...
	public Group getGroupDetails(String groupName) {
		...
		ModelUtils.assertPropertyEquality(returnedGroupExpectation, returnedGroup, 5, Pattern.compile(".*users.*\\.(apiKey|homepage|realname|email|password|date|openURL|gender|place|interests|hobbies|IPAddress|basket|profession|place|spammer|settings)|.*\\.date|.*\\.scraperId|.*\\.openURL|.*groupId"));
		EasyMock.verify(serverLogic);
		assertLogin();
	...
	public Post<? extends org.bibsonomy.model.Resource> getPostDetails(String resourceHash, String userName) {
		...
		final String[] ignoreProperties = new String[] {"date", "user.apiKey", "user.email", "user.homepage", "user.password", "user.realname", "resource.scraperId", "resource.openURL", "user.IPAddress", "user.basket", "user.gender", "user.interests", "user.hobbies", "user.profession", "user.openURL", "user.place", "user.spammer", "user.settings"};
	...
	public <T extends org.bibsonomy.model.Resource> List<Post<T>> getPosts(Class<T> resourceType, GroupingEntity grouping, String groupingName, List<String> tags, String hash, Order order, int start, int end, String search) {
		...
		ModelUtils.assertPropertyEquality(expectedPosts, returnedPosts, 5, Pattern.compile(".*\\.user\\.(apiKey|homepage|realname|email|password|date|openURL|gender|place|interests|hobbies|IPAddress|basket|profession|place|spammer|settings)|.*\\.date|.*\\.scraperId|.*\\.openURL"));
	...
	public User getUserDetails(String userName) {
		...
		ModelUtils.assertPropertyEquality(expected, returned, 3, null, "apiKey", "email", "homepage", "password", "realname", "date", "openURL", "gender", "place", "IPAddress", "basket", "profession", "place", "spammer", "settings", "hobbies", "interests");
	...
	public List<User> getUsers(int start, int end) {
		...
		ModelUtils.assertPropertyEquality(expected, returned, 5, Pattern.compile(".*\\.(apiKey|homepage|realname|email|password|date|openURL|gender|place|interests|hobbies|IPAddress|basket|profession|place|spammer|settings)"));
		EasyMock.verify(serverLogic);
		assertLogin();
		return returned;
	}
