	...
	public View workOn(final OpenAccessCommand command) {
		...
		if (present(action)) {
			if (GET_SENT_REPOSITORIES.equals(action)) {
				...
				final List<Post<BibTex>> posts = logic.getPosts(BibTex.class, GroupingEntity.USER, command.getContext().getLoginUser().getName(), null, command.getInterhash(), null, QueryScope.LOCAL, Sets.<Filter>asSet(FilterEntity.POSTS_WITH_REPOSITORY), SortKey.NONE, null, null, 0, this.maxQuerySize);
