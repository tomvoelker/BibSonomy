...
public class Ldap implements ObjectFactory, InitialContextFactory, Serializable {
	...
	public Object getObjectInstance(Object obj, Name name, Context nameCtx, Hashtable<?, ?> environment) throws NamingException {
		...
		while (addrs.hasMoreElements()) {
			...
			// log.info("-----------------------------------------------------------------           LDAP-JNDI: "
			// + addr.getType().toString() + " = " +
			// addr.getContent().toString());
			if (addr.getType().equals("java.naming.factory.initial")) {
				...
				// log.info(" - - - LDAP-JNDI: " + addr.getType().toString() +
				// " = " + addr.getContent().toString());
			}
	...
	/**
	 * check authorization of user returns ldapuserinfo object with retrieved
	 * userdata if user exists otherwise null
	 * 
	 * @param ldapUserId
	...
	public LdapUserinfo checkauth(String ldapUserId, String ldapCredentials) {
		...
		// query += "("+ ldapUserPasswordField
		// +"="+ldapUserinfo.generatePicaHashBase64(ldapCredentials)+")";
		// query += ")";
