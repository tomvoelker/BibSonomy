...
public class PublicationAutocompleteController implements MinimalisticController<PublicationAutocompleteCommand> {
	...
	public View workOn(final PublicationAutocompleteCommand command) {
		...
		if (present(isbn)) {
			...
			try {
				final List<Post<BibTex>> postsBySearch = this.logic.getPosts(BibTex.class, GroupingEntity.ALL, null, tags, null, search, QueryScope.LOCAL, null, SortKey.RANK, null, null, 0, 10);
