...
public class ItemRecommenderFeedbackController extends AjaxController implements MinimalisticController<AjaxItemRecommenderFeedbackCommand>{
	...
	public View workOn(final AjaxItemRecommenderFeedbackCommand command) {
		...
		if (command.getAction().equalsIgnoreCase(ACTION_BIBTEX)) {
			List<Post<BibTex>> posts = this.logic.getPosts(BibTex.class, GroupingEntity.USER, command.getUserName(), null, command.getIntraHash(), null, QueryScope.LOCAL, null, SortKey.NONE, null, null, 0, 1);
			if (present(posts)) {
			...
			List<Post<Bookmark>> posts = this.logic.getPosts(Bookmark.class, GroupingEntity.USER, command.getUserName(), null, command.getIntraHash(), null, QueryScope.LOCAL, null, SortKey.NONE, null, null, 0, 1);
			if (present(posts)) {
