...
public class TagStringParser extends antlr.LLkParser       implements TagStringParserTokenTypes
 {
...
public TagStringParser(ParserSharedInputState state) {
  ...
  tokenNames = _tokenNames;
	...
	public final void tagstring() throws RecognitionException, TokenStreamException {
	...
	public final void uprel() throws RecognitionException, TokenStreamException {
		...
		try {      // for error handling
			match(LEFTARROW);
			t=tag();
			tagobj.addTagRelation(t, lasttag); tagobj.addTag(t); tagobj.addTag(lasttag); lasttag=t;
		}
		catch (RecognitionException ex) {
			reportError(ex);
	...
	public final void lorel() throws RecognitionException, TokenStreamException {
		...
		try {      // for error handling
			match(RIGHTARROW);
			t=tag();
			tagobj.addTagRelation(lasttag, t); tagobj.addTag(t); tagobj.addTag(lasttag); lasttag=t;
		}
		catch (RecognitionException ex) {
			reportError(ex);
			recover(ex,_tokenSet_2);
