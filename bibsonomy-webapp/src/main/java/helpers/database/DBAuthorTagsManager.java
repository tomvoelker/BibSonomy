...
public class DBAuthorTagsManager extends DBManager{
	...
	/*private static final String SQL_SELECT_TAS_ALL          = "SELECT tag_name, count(tag_name) AS tag_count FROM tas t, search s " 
															+ "WHERE MATCH(s.author) AGAINST (? IN BOOLEAN MODE) AND s.content_type = 2 AND "
															+ "AND t.content_id = s.content_id GROUP BY t.tag_name ";*/
	private static final String SQL_SELECT_TAS_PUBLIC       = "SELECT tag_name, count(tag_name) AS tag_count FROM tas t, search s " 
															+ "WHERE MATCH(s.author) AGAINST (? IN BOOLEAN MODE) AND s.content_type = 2 AND s.group = 0 "
															+ "AND t.content_id = s.content_id GROUP BY t.tag_name ";
