...
public class BookmarkUpdate{
	...
	public int update(HashSet<String> db_bookmark_keys)throws Exception{
		...
		handleEntries(db_bookmark_keys, presult.getWwwlist());
		handleEntries(db_bookmark_keys, presult.getInsert_incomplete_author_editor());

		return presult.getException().size();
	}

	private void handleEntries(HashSet<String> db_bookmark_keys, LinkedList<DBLPEntry> entryList) {
			...
			if(entry.getEntrydate().after(presult.getDblpdate())){
				...
				if(list.size() >= maxNumberOfEntries){
					try{
						...
						System.out.println("DBLPUpdater: BookmarkUpdate: " + e);
						e.printStackTrace();
					}
					list.clear();
				}
