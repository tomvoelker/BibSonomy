		...
		try {
			...
			if (requGroup == null && "true".equals(cancel)) {
				...
				if (captcha == null) { 
					...
					request.setAttribute("error","Wrong code: Please try again");
				...
				if (rst.next()) {
					...
					if (!rst.next()) {
						...
						if (rst.next()) {
							...
							"\nReason: " + reason + "\n" + 
							"\nVisit " + projectHome + "settings.jsp?group="+URLEncoder.encode(requGroup, "UTF-8").toLowerCase()+"&user="+URLEncoder.encode(currUser, "UTF-8").toLowerCase()+
							"\n"+
							"\nHave a look on the help page and the FAQ:" +
							"\n" + projectHome + "help" +
							"\n" + projectHome + "faq" + "\n" +
