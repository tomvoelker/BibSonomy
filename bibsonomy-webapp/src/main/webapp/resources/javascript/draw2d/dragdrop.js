...
Draggable.prototype.construct = function (oElement /*: HTMLElement */, 
    ...
    var fnTemp = function () {
        ...
        */
        ...
        //
        oDragStartEvent.x = oEvent.clientX - oThis.element.offsetLeft+document.body.parentNode.scrollLeft;
        oDragStartEvent.y = oEvent.clientY - oThis.element.offsetTop+document.body.parentNode.scrollTop;
...
Draggable.prototype.attachEventHandlers = function () {
    ...
    this.tempMouseMove = function () {
        ...
        {
         var snapPoint = oThis.node.getSnapToGridAnchor();
         
         iNewX= oThis.node.getWorkflow().snapToGridX(iNewX+snapPoint.x)-snapPoint.x;
         iNewY= oThis.node.getWorkflow().snapToGridY(iNewY+snapPoint.y)-snapPoint.y;
        }
...
Draggable.prototype.getDropTarget = function (/*:int*/ x ,/*:int*/  y ){
  ...
  for(key in this.targets)
  {
    ...
    if (target!=null && target.node.isOver(x, y) && target.node!=this.node)
    {
        return target;
    }
  }
...
Draggable.prototype.getCompartment = function (x /*: int */, y /*: int */) /*: DropTarget */ 
{
  ...
  for(key in this.node.workflow.compartments)
  {
    ...
    if (target!=null && target.isOver(x, y) && target!=this.node)
    {
        if(result==null)
           result = target;
