...
function suggest() {
    ...
    for (var tagToCopy in copyListElements) {
        var duplicate = false;
        var tmpTag = "";
        valid = true;
        if(searchLength <= tagToCopy.length)
            tmpTag = tagToCopy.substring(0,searchLength).toLowerCase();
        else
            valid = false;
        ...
        if(tmpTag.match(activeTag) && valid) {
            for(var z in tags) {
                ...
                if(tagToCopy.toLowerCase() == tags[z].toLowerCase()) {
                    duplicate = true;
                    break;
                }
            }
            if(!duplicate)
                copyCollect.push(tagToCopy);
        }
        i++;
    }
