...
<%-------------------------- Heading -----------------------%>
<h1 id="path"><a href="/" rel="Start">${projectName}</a> :: <a rel="path_menu" href="/admin_statistics.jsp">statistics&nbsp;<img src="/resources/image/box_arrow.png"></a></h1>
...
function hideNextDiv(evt) {
  var node = xget_event(evt);
  
  while (node && node.nodeName.toUpperCase() != "DIV") {
  ...
  if (node) {
    if (node.style.display != "none") {
      ...
      <span class="hide" onclick="hideNextDiv(event);">hide</span>
      <div class="histo">
        bookmarks
        <c:forEach var="col" items="${stats.bookmarkUserHisto}">
          <span style="height: ${col.value}px; left: ${col.key*2}px;"><c:if test="${col.key gt 1000}">${col.key}:${col.value}</c:if></span>
        </c:forEach>
      </div>
      ...
      <div class="histo">
        publications
        <c:forEach var="col" items="${stats.publicationUserHisto}">
          <span style="height: ${col.value}px; left: ${col.key*2}px;"><c:if test="${col.key gt 1000}">${col.key}:${col.value}</c:if></span>
        </c:forEach>
      </div>
