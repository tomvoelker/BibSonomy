...
public class HTTPBookmarkUpdate extends HTTPUpdate {			
	 ...
	 */
	public void deleteOldBookmarkByEntry(LinkedList<DBLPEntry> list, DBHandler handler, HashSet<String> db_bookmark_keys) throws InstantiationException, IllegalAccessException, ClassNotFoundException, SQLException, DBLPException {
		handler.open(); 

		for (DBLPEntry entry: list) {
			if (db_bookmark_keys.contains(entry.getDblpKey())) {
				String urlhash = handler.getUrlHash(entry.getDblpKey());
				if (urlhash != null) {
					try {
						deletePost(urlhash);
					} catch (IOException e) {
	...
	public void deleteOldBookmarkByKey(LinkedList<String> keys, DBHandler handler) throws InstantiationException, IllegalAccessException, ClassNotFoundException, SQLException, DBLPException {
		...
		handler.open();
		for (String key: keys) {
			String hash = handler.getUrlHash(key);
	...
	public void insertNewBookmark(LinkedList<DBLPEntry> entrylist, DBLPParseResult presult) throws DBLPException {
		for (DBLPEntry entry: entrylist) {
			try {
				...
				System.out.println("BookmarkInsert: " + e);
				e.printStackTrace();
				try {
					presult.getException().add("insert bookmark=" + e.toString() + " " + entry.generateSnippet());
				} catch(Exception nokey) {
					presult.getException().add("insert bookmark=" + e.toString());
				}
			}
		}
	}
