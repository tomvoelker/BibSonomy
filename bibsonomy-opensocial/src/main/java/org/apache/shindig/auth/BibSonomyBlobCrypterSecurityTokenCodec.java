 ...
 * 
 * FIXME: THIS IS A TEMPRORARY IMPLEMENTATION FOR BOOTSTRAPPING
 *
 * @since 2.0.0
 */
@Singleton
public class BibSonomyBlobCrypterSecurityTokenCodec implements SecurityTokenCodec {
  ...
  public BibSonomyBlobCrypterSecurityTokenCodec(ContainerConfig config) {
	  for (String container : config.getContainers()) {
		  String keyString = config.getString(container, SECURITY_TOKEN_KEY_STRING);
		  if (keyString!= null) {
