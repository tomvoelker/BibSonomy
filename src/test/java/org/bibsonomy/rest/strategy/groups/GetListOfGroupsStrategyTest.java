...
public class GetListOfGroupsStrategyTest extends TestCase
{
   ...
   {
      Context c = new Context( new TestDatabase(), "get", "/groups", new HashMap<String,String>() );
      ...
      // renderer test
      assertEquals(  332, response.getStringWriter().toString().length() );
      assertEquals( "text/xml", c.getContentType( "firefox" ) );
      assertEquals( "bibsonomy/groups+XML", c.getContentType( Context.API_USER_AGENT ) );
   }
}
