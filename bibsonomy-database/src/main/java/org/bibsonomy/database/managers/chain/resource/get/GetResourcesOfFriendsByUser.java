...
public class GetResourcesOfFriendsByUser<R extends Resource, P extends ResourceParam<R>> extends ResourceChainElement<R, P> {
	...
	protected List<Post<R>> handle(final P param, final DBSession session) {
		...
		if (this.generalDb.isFriendOf(param.getUserName(), param.getRequestedUserName(), session)) {
			...
			return this.databaseManager.getPostsForUser(param.getUserName(), param.getRequestedUserName(), HashID.getSimHash(param.getSimHash()), groupId, param.getGroups(), param.getFilter(), param.getLimit(), param.getOffset(), param.getSystemTags(), session);
		}
