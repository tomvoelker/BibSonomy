...
public class GroupDatabaseManager extends AbstractDatabaseManager {
	...
	public void activateGroup(final String group, final DBSession session) {
		try {
			...
			// add the dummy user to the group
	...
	public void createGroup(final Group group, final DBSession session) {
		...
		if (!present(groupUser.getName())) {
			// create the user
			// TODO: move this to a UserUtils.generateDummyUser(normedGroupName) method
			u.setRealname("autogenerated");
			u.setPassword(UserUtils.generateRandomPassword());
		...
		try {
			...
			this.userDb.activateUser(u, session);
	...
	/**
	 * Removes the group from the pending group table.
	 * 
	 * @param groupname
	 * @param session
	 */
