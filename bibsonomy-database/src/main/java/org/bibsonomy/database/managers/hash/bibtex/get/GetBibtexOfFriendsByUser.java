 ...
 * @version $Id$
 */
public class GetBibtexOfFriendsByUser extends BibTexHashElement {

	public GetBibtexOfFriendsByUser() {
	...
	protected boolean canHandle(final BibTexParam param) {
		return present(param.getUserName()) && !present(param.getBibtexKey()) && (param.getGrouping() == GroupingEntity.FRIEND) && present(param.getRequestedUserName()) && !present(param.getTagIndex()) && !present(param.getHash()) && nullOrEqual(param.getOrder(), Order.ADDED) && !present(param.getSearch());
	}
}