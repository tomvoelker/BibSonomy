...
public class GetGlobalTasCount extends StatisticChainElement {
	...
	protected boolean canHandle(StatisticsParam param) {
		return GroupingEntity.ALL.equals(param.getGrouping()) && !present(param.getStatisticsConstraint());
	}

	@Override
	protected Statistics handle(StatisticsParam param, DBSession session) {
		return new Statistics(this.db.getNumberOfTas(param.getContentType(), session));
	}
