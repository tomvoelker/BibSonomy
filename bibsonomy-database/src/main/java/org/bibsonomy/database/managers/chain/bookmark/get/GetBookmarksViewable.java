...
public class GetBookmarksViewable extends BookmarkChainElement {
	...
	protected List<Post<Bookmark>> handle(final BookmarkParam param, final DBSession session) {
		...
		if (present(param.getTagIndex())) {
			return this.db.getPostsViewableByTag(param.getUserName(), param.getRequestedUserName(), param.getTagIndex(), groupId, param.getFilter(), param.getLimit(), param.getOffset(), param.getSystemTags().values(), session);
		}
