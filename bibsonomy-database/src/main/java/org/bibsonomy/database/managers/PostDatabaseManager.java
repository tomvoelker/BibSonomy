...
public abstract class PostDatabaseManager<R extends Resource, P extends ResourceParam<R>> extends AbstractDatabaseManager implements CrudableContent<R, P> {
	 ...
	 *
	 */
	public List<Post<R>> getPostsByTagNames(final int groupId, final List<TagIndex> tagIndex, final QueryScope queryScope, final Order order, final int limit, final int offset, final DBSession session) {
		if(QueryScope.FEDERATED == queryScope){
			final List<String> tagIndexNames = new ArrayList<String>();
			for(final TagIndex tag:tagIndex){
				tagIndexNames.add(tag.getTagName());
			}
			return this.resourceSearch.getPosts(null, null, null, null, null, queryScope, null, null, null, null, tagIndexNames, null, null, null, null, order, limit, offset);
	 ...
	 * @return list of posts
	 */
	public List<Post<R>> getPostsForHomepage(final Set<Filter> filters, final Date startDate, final Date endDate, final int limit, final int offset, final Collection<SystemTag> systemTags, final DBSession session) {
		final P param = this.createParam(limit, offset);
		param.setGroupId(GroupID.PUBLIC);
	...
	public List<Post<R>> getPostsByResourceSearch(final String userName, final String requestedUserName, final String requestedGroupName, final List<String> requestedRelationName, final Collection<String> allowedGroups, final QueryScope queryScope, final String searchTerms, final String titleSearchTerms, final String authorSearchTerms, final Collection<String> tagIndex, final String year, final String firstYear, final String lastYear, final List<String> negatedTags, final Order order, final int limit, final int offset) {
		if (present(this.resourceSearch)) {
			if (present(queryScope)){
				return this.resourceSearch.getPosts(userName, requestedUserName, requestedGroupName, requestedRelationName, allowedGroups, queryScope, searchTerms, titleSearchTerms, authorSearchTerms, null, tagIndex, year, firstYear, lastYear, negatedTags, order, limit, offset);
			}
	...
	public List<Post<R>> getPostsViewable(final String requestedGroupName, final String loginUserName, final int groupId, final HashID simHash, final int limit, final int offset, final Collection<SystemTag> systemTags, final DBSession session) {
		if (GroupID.isSpecialGroupId(groupId)) {
			...
			// he as full access to his/her own posts
			return this.getPostsForUser(loginUserName, loginUserName, HashID.INTER_HASH, groupId, new LinkedList<Integer>(), PostAccess.FULL, null, limit, offset, null, session);
		}
	 ...
	 * @return list of posts
	 */
	public List<Post<R>> getPostsViewableByTag(final String requestedGroupName, final String loginUserName, final List<TagIndex> tagIndex, final int groupId, final Set<Filter> filters, final int limit, final int offset, final Collection<SystemTag> systemTags, final DBSession session) {
		if (GroupID.isSpecialGroupId(groupId)) {
			// show users own posts; has full access to his/her own posts
			return this.getPostsByTagNamesForUser(loginUserName, loginUserName, tagIndex, groupId, Collections.<Integer>emptyList(), limit, offset, PostAccess.FULL, filters, systemTags, session);
		}
	 ...
	 * @return list of posts
	 */
	public List<Post<R>> getPostsForGroup(final int groupId, final List<Integer> visibleGroupIDs, final String loginUserName, final HashID simHash, final PostAccess postAccess, final Set<Filter> filters, final int limit, final int offset, final Collection<SystemTag> systemTags, final DBSession session) {
		final P param = this.createParam(loginUserName, null, limit, offset);
		param.setGroupId(groupId);
		param.setGroups(visibleGroupIDs);
		param.setSimHash(simHash);
	 ...
	 * @return list of posts
	 */
	public List<Post<R>> getPostsForGroupByTag(final int groupId, final List<Integer> visibleGroupIDs, final String loginUserName, final List<TagIndex> tagIndex, final PostAccess postAccess, final Set<Filter> filters, final int limit, final int offset, final Collection<SystemTag> systemTags, final DBSession session) {
		final P param = this.createParam(loginUserName, null, limit, offset);
		param.setGroupId(groupId);
		param.setGroups(visibleGroupIDs);
		param.setTagIndex(tagIndex);
	 ...
	 * @return list of posts
	 */
	public List<Post<R>> getPostsForUser(final String loginUserName, final String requestedUserName, final QueryScope queryScope, final HashID simHash, final int groupId, final List<Integer> visibleGroupIDs, final PostAccess postAccess, final Set<Filter> filters, final int limit, final int offset, final Collection<SystemTag> systemTags, final DBSession session) {
		if(queryScope == QueryScope.FEDERATED){
			return this.resourceSearch.getPosts(loginUserName, requestedUserName, null, null, null, queryScope, null, null, null, null, null, null, null, null, null, null, limit, offset);
		}
	 ...
	 * @return list of posts
	 */
	public List<Post<R>> getPostsForUser(final String loginUserName, final String requestedUserName, final HashID simHash, final int groupId, final List<Integer> visibleGroupIDs, final PostAccess postAccess, final Set<Filter> filters, final int limit, final int offset, final Collection<SystemTag> systemTags, final DBSession session) {
		final P param = this.createParam(loginUserName, requestedUserName, limit, offset);
		param.setGroupId(groupId);
		param.setGroups(visibleGroupIDs);
		param.setSimHash(simHash);
	 ...
	 * @return list of posts
	 */
	public List<Post<R>> getPostsWithDiscussions(final String loginUserName, final String requestedUserName, final List<Integer> visibleGroupIDs, final Set<Filter> filters, final int limit, final int offset, final Collection<SystemTag> systemTags, final DBSession session) {
		final P param;
	 ...
	 * @return list of posts
	 */
	public List<Post<R>> getPostsWithDiscussionsForGroup(final String loginUserName, final int requestedGroupId, final List<Integer> visibleGroupIDs, final Set<Filter> filters, final int limit, final int offset, final Collection<SystemTag> systemTags, final DBSession session) {
		final P param;
		//group param
		...
		try {
			...
			if (present(newPostInDB)) {
				...
				if (!intraHash.equals(oldHash)) {
					 ...
					 * way we would delete the post with the old hash and post
	...
	// * This includes updating groupID in three tables:1. bibtex or bookmark,
	// 2. tas, 3. groupTas
	// * which is done in the following methods:
	// * updatePostGroups(param, session) : updates 1
	// * tagDb.deleteGroupsOfTags(oldPost, session),
	// tagDb.insertGroupsOfTags(post, session): updates 2 & 3
	// **/
	// private void performUpdateOnlyPrivacy(final Post<R> post, final Post<R>
	// oldPost, final DBSession session) {
	// session.beginTransaction();
	// try {
	//
	// /*
	...
	// **/
	// private void performUpdateOnlyNormalize(final Post<R> post, final Post<R>
	// oldPost, final DBSession session) {
	// session.beginTransaction();
	// try {
	//
	...
	// */
	// protected void updatePostBibTexKey(final P param, final DBSession
	// session) {
	// session.beginTransaction();
	// try {
	// // updates bibtexkey
		...
		for (final HashID hashId : this.getHashRange()) {
			...
			if (present(hash)) {
				...
				if (delete) {
					...
					// insert new hash or increment its counter, if it already
					// exists
