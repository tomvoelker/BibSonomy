...
public class GetBookmarksViewable extends RequestHandlerForGetPosts{
	...
	/*
	 * return a list of bookmark by a given group (which is only viewable for groupmembers excluded public option regarding setting a post).
	...
	protected List<Post<? extends Resource>> handleRequestForGetPosts(String authUser, GroupingEntity grouping, String groupingName, List<String> tags, String hash, boolean popular, boolean added, int start, int end) {
		...
		List<Post<? extends Resource>> posts = db.bookmarkDatabaseManager.bookmarkList("getBookmarkViewable", param, true);
		return posts;
	}
