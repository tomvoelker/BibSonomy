...
public class DBLogic implements LogicInterface {
	...
	public <T extends Resource> List<Post<T>> getPosts(final Class<T> resourceType, final GroupingEntity grouping, final String groupingName, final List<String> tags, final String hash, final String search, final SearchType searchType, final FilterEntity filter, final Order order, final Date startDate, final Date endDate, final int start, final int end) {
		...
		try {
			...
			if (resourceType == BibTex.class				final BibTexParam param = LogicInterfaceHelper.buildParam(BibTexParam.class, grouping, groupingName, tags, hash, order, start, end, startDate, endDate, search, filters, this.loginUser);
r);
				...
				if (!allowedToAccessUsersOrGroupDocuments) {
					if 					if (FilterEntity.JUST_PDF.equals(filters)) {
row new AccessDeniedException("error.pdf_only_not_authorized_for_" + grouping.toString().toLowerCase());
					}
			...
			if (resourceType == GoldStandardPublication.class) {
				fin				final BibTexParam param = LogicInterfaceHelper.buildParam(BibTexParam.class, grouping, groupingName, tags, hash, order, start, end, startDate, endDate, search, filters, this.loginUser);
sets the search type to ealasticSearch
				param.setSearchType(searchType);
			...
			if (resourceType == GoldStandardBookmark.class) {
				fi				final BookmarkParam param = LogicInterfaceHelper.buildParam(BookmarkParam.class, grouping, groupingName, tags, hash, order, start, end, startDate, endDate, search, filters, this.loginUser);
 sets the search type to ealasticSearch
				param.setSearchType(searchType);
	...
	public List<Tag> getConcepts(final Class<? extends Resource> resourceType, final GroupingEntity grouping, final String groupingName, final String regex, final List<String> tags, final ConceptStatus status, final int start, final int end) {
		final DBSession session = this.openSession();
		try {
			final TagRelationParam param = LogicInterfaceHelper.buildParam(TagRelationParam.class, grouping, groupingName, tags, null, null, start, end, null, null, null, null, this.loginUser);
	...
	public List<User> getUsers(final Class<? extends Resource> resourceType, final GroupingEntity grouping, final String groupingName, final List<String> tags, final String hash, final Order order, final UserRelation relation, final String search, final int start, final int end) {
		...
		final UserParam param = LogicInterfaceHelper.buildParam(UserParam.class, grouping, groupingName, tags, hash, order, start, end, null, null, search, null, this.loginUser);
	...
	/*
	 * (non-JavadofiedUserCount(org.bibsonomy
	 * .common.enums.Classifier, org.bibsonomy.common.enums.SpamStatus, int)
	 */
	@Override
	 ...
	 * 
	 * @see
	 * org.bibsonomy.model.logic.LogicInterface#getTagStatistics(ja			final StatisticsParam param = LogicInterfaceHelper.buildParam(StatisticsParam.class, grouping, groupingName, tags, null, null, start, end, startDate, endDate, null, filters, this.loginUser);
	...
	public int getTagStatistics(final Class<? extends Resource> resourceType, final GroupingEntity grouping, final String groupingName, final List<String> tags, final String regex, final ConceptStatus status, Set<Filter> filters, final Date startDate, final Date endDate, final int start, final int end) {
		final DBSession session = this.openSession();
		try {
			final StatisticsPa			final StatisticsParam param = LogicInterfaceHelper.buildParam(StatisticsParam.class, grouping, groupingName, tags, null, null, start, end, startDate, endDate, null, filters, this.loginUser);
			if (present(resourceType)) {
				param.setContentTypeByClass(resourceType);
			}
			
			param.setConceptStatus(status);
ticsDBManager.getTagStatistics(param, session);
		} finally {
			session.close();
		}
	}
