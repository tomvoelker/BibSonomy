...
public class DBLogic implements LogicInterface {
	...
	public void updateSyncData(final String userName, final URI service, final Class<? extends Resource> resourceType, final Date syncDate, final SynchronizationStatus status, final String info) {
		...
		final DBSession session = this.openSession();
		try {
			this.syncDBManager.updateSyncData(userName, service, resourceType, syncDate, status, info, session);
		} finally {
			session.close();
		}
	}
	...
	public Group getGroupDetails(final String groupName) {
		final DBSession session = this.openSession();
		try {
			final boolean getPermissions = this.permissionDBManager.isAdmin(this.loginUser) ? true : false;
			final Group myGroup = this.groupDBManager.getGroupMembers(this.loginUser.getName(), groupName, getPermissions, session);
	...
	public String updateGroup(final Group paramGroup, final GroupUpdateOperation operation, final GroupMembership membership) {
		...
		try {
			...
			case REMOVE_MEMBER:
				 ...
				 * this.groupDBManager.removeUserFromGroup(group.getName(),
				 * membership.getUser().getName(), session);
				 * }
				 * break;
				 */
	...
	private <T extends Resource> String createPost(final Post<T> post, final DBSession session) {
		...
		if (Role.SYNC.equals(this.loginUser.getRole())) {
			this.validateGroupsForSynchronization(post);
		}
