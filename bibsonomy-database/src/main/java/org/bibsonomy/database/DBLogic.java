...
public class DBLogic implements LogicInterface {
	...
	public <T extends Resource> List<Post<T>> getPosts(final Class<T> resourceType, final GroupingEntity grouping, final String groupingName, final List<String> tags, final String hash, final String search, final SearchType searchType, final FilterEntity filter, final Order order, final Date startDate, final Date endDate, final int start, final int end) {
		...
		try {
			 ...
			 * groupingName, tags, hash, popular, added, start, end, false));
			 */
			if (FilterEntity.HISTORY.equals(filter) && !((resourceType == GoldStandardPublication.class) || (resourceType == GoldStandardBookmark.class))) {
	 ...
	 */
	@Override
	public Statistics getUserStatistics(GroupingEntity grouping, Set<StatisticsConstraint> constraints, FilterEntity filter, Classifier classifier, SpamStatus status, Integer interval, final StatisticsUnit unit) {
		final DBSession session = openSession();
		try {
			return this.statisticsDBManager.getUserStatistics(grouping, constraints, filter, classifier, status, interval, unit, session);
		} finally {
			session.close();
		}
	}
