 ...
 * @version $Id$
 */
public class TwoStepSynchronizationClientTest extends AbstractDatabaseManagerTest {
	...
	private void syncResources(final TwoStepSynchronizationClient sync, final URI syncServer, final Class<? extends Resource> resourceType, final String[] keys, final List<SynchronizationPost> syncPlan) {
		 ...
		 * do sync
		 */
		final SynchronizationData data = sync.synchronize(clientLogic, serverLogic, serverUser.getName(), resourceType, SynchronizationDirection.BOTH, syncPlan);
