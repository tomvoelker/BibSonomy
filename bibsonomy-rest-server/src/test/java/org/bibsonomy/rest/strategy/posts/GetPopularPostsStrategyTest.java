...
public class GetPopularPostsStrategyTest extends AbstractContextTest {
	...
	public void testGetPopularPostsStrategy() throws Exception {
		final Context c = new Context(HttpMethod.GET, "/api/posts/popular", RenderingFormat.XML, new RendererFactory(this.urlRenderer), this.is, null, this.db, null, new HashMap<String, String>(), null);
		final ByteArrayOutputStream baos = new ByteArrayOutputStream();
		c.perform(baos);

		// just test length, because the detail rendering output is tested by
		// the renderer test
		assertEquals(11254, baos.toString().length());
		assertEquals("text/xml", c.getContentType("firefox"));
		assertEquals("bibsonomy/posts+XML", c.getContentType(RESTConfig.API_USER_AGENT));
	}
}