...
public class GetListOfTagsStrategyTest extends AbstractContextTest {
	...
	public void testGetListOfTagsStrategy() throws Exception {
		final Context ctx = new Context(HttpMethod.GET, "/tags", RenderingFormat.XML, this.is, null, this.db, new HashMap<String, String>(), null);
		final ByteArrayOutputStream baos = new ByteArrayOutputStream();
		ctx.perform(baos);

		// just test length, because the detail rendering output is tested by
		// the renderer test
		assertEquals(1595, baos.toString().length());
