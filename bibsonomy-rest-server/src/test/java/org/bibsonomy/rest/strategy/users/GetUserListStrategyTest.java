...
public class GetUserListStrategyTest extends AbstractContextTest {
	...
	public void testGetUserListStrategy() throws Exception {
		final Context ctx = new Context(HttpMethod.GET, "/users", RenderingFormat.XML, this.is, null, this.db, new HashMap<String, String>(), null);
		final ByteArrayOutputStream baos = new ByteArrayOutputStream();
		ctx.perform(baos);

		// just test length, because the detail rendering output is tested by
		// the renderer test
		assertEquals(1210, baos.toString().length());
