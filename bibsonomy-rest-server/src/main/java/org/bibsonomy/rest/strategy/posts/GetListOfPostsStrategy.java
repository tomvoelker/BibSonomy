...
public class GetListOfPostsStrategy extends Strategy {
	...
	public void perform(final HttpServletRequest request, final Writer writer) throws InternServerException {
		...
		final List<? extends Post<? extends Resource>> posts = this.context.getLogic().getPosts(this.context.getAuthUserName(), resourceType, grouping, groupingValue, this.context.getTags("tags"), hash, null, start, end);
		final ViewModel viewModel = new ViewModel();
		if (posts.size() < end || posts.size() > end) {
			end = posts.size();
		} else {
			String next = RestProperties.getInstance().getApiUrl() + RestProperties.getInstance().getPostsUrl() + "?start=" + String.valueOf(end) + "&end=" + String.valueOf(end + 20);
