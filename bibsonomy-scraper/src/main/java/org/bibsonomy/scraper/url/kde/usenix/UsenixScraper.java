...
public class UsenixScraper extends AbstractUrlScraper {
	...
	private static final Pattern PATTERN_BIBTEX = Pattern.compile("<div class=\"bibtex-text-entry bibtex-accordion-text-entry\">([\\s\\S]*?)</div>");
	private static final Pattern PATTERN_YEAR_EVENTS = Pattern.compile("/events/.*?(\\d{2,4})/");
	private static final Pattern PATTERN_YEAR_PROCEEDING = Pattern.compile("/publications/library/proceedings/\\D*(\\d{2,4})/");
	...
	static {
		patterns.add(new Pair<Pattern, Pattern>(Pattern.compile(".*" + SITE_HOST), EMPTY_PATTERN));
	}
	...
	protected boolean scrapeInternal(ScrapingContext sc)throws ScrapingException {
		sc.setScraper(this);
		URL redirectUrl = WebUtils.getRedirectUrl(sc.getUrl());
		boolean isLegacyPage = sc.getUrl().getPath().contains("legacy")||(redirectUrl!=null&&redirectUrl.getPath().contains("legacy"));

		Matcher m_bibtex = PATTERN_BIBTEX.matcher(sc.getPageContent());
		if (!isLegacyPage && m_bibtex.find()){
			String bibtex = m_bibtex.group(1).replaceAll("<br />", "");
			sc.setBibtexResult(bibtex);
			return true;
		}

		if (isLegacyPage&&(sc.getUrl().getPath().contains("/events/")||sc.getUrl().getPath().contains("/publications/library/proceedings/"))){
			...
			try {
				...
				}else if(path.contains("/publications/library/proceedings/")){
					// get year
