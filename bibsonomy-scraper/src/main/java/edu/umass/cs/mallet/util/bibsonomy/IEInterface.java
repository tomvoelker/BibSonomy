...
/**
 * modified: Sep 29, 2006
 *
 * modified for bibsonomy requirements 
 * 
...
//		System.out.println("==================end of crf ==============");
	...
	public boolean loadCRF(CRF4 crf)
	{
		this.crf = crf;
		this.pipe = (SerialPipes) crf.getInputPipe();
	...
	public SortedMap<String, String> printResultInHashMap(Sequence viterbiSequence, TokenSequence tokenSequence)
	{
		...
		for(int i=0; i<tokenSequence.size(); i++){
			...
			if(tag != old_tag){
				if(old_tag != null){
					String entry = "";
					if(map.get(old_tag) == null){
						map.put(old_tag, element);// save Element
						entry = map.get(old_tag);
					}else{
						map.put(old_tag, map.get(old_tag) + element);// save Element
						entry = map.get(old_tag); 
					}
					element = "";
				}
			...
			// add word to element with whitespace separation
			if(!element.equals(""))
				element = element + getWhitespace(previousWord, word) + word;
			else
				element += word;
						
			if(i == tokenSequence.size() - 1){
				if(map.get(tag) == null)
					map.put(tag, element);// save Element
				else
					map.put(tag, map.get(tag) + element);// save Element
				}
	...
	private String getWhitespace(String previousWord, String currentWord){
		//check input
		if(currentWord == null)
			return "";
		...
		//bevor and after " no whitespace appears 
		if(previousWord != null && ( currentWord.equals("\"") || previousWord.equals("\"")) )
			return "";
		//bevor and after _ no whitespace appears 
		if(previousWord != null && ( currentWord.equals("_") || previousWord.equals("_")) )
			return "";
		//bevor and after - no whitespace appears 
		if(previousWord != null && ( currentWord.equals("-") || previousWord.equals("-")) )
			return "";
		
		// bevor and after "(" or ")" no whitespace appears
		if(previousWord != null && ( previousWord.equals("(") || currentWord.equals("(") || previousWord.equals(")") || currentWord.equals(")")) )
			return "";
		//bevor and after "[" or "]" no whitespace appears
		if(previousWord != null && ( previousWord.equals("[") || currentWord.equals("[") || previousWord.equals("]") || currentWord.equals("]")) )
			return "";
		...
		// rest is speratet with whitespace
		return " ";
	}
	...
	private SortedMap<String, String> cleanUp(final SortedMap<String, String> map){
		...
		for (final String field: map.keySet()){
			...
			if(value!=null && value.length()>4){
				...
				if(value.charAt(value.length()-1) == '.' && (value.charAt(value.length()-2) == ' ' || (value.charAt(value.length()-3) != ' ' && value.charAt(value.length()-4) != ' ' )))//remove last char '.', not if String bevor a initial or bigram(like the "al" of "et al.")
