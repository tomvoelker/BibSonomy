...
/**
 * Class for querying several recommenders. Each recommendation request is sent
 * to all registered recommenders. Thereby each query to an recommendation is
 * identified by an unique transaction id which is used to manage asynchronous
 * events. Responses of the recommenders are collected using the
 * consumer/producer schema. The overall result is chosen in selectResult().
 * Request, all received responses as well as timing and individual recommender
 * meta information is stored using class DBAccess.
 * 
 * @author fei
 */
public class MultiplexingTagRecommender implements TagRecommender {
    private static final Log log = LogFactory
	    .getLog(MultiplexingTagRecommender.class);
    Object lockResults = new Object(); // while selecting a result,
    // no further recommender answers
    // should be added to database
    private List<TagRecommender> localRecommenders; // recommenders with object
    // reference
    private List<TagRecommenderConnector> distRecommenders; // recommenders with
    // remote access
	...
	for (TagRecommender rec : getLocalRecommenders()) {
	    // each recommender is identified by an unique id
	    registerRecommender(rec, rec.getClass().getCanonicalName(), rec
		    .getInfo(), null);
	}
	...
	if (initialized) {
	    registerRecommender(recommender, recommender.getClass()
		    .getCanonicalName(), recommender.getInfo(), null);
	}
	return true;
    }
    ...
    /**
     * @return false if one of the registered recommenders could not be
     *         disconnected
     */
     ...
     * recommender queries to posts in BibSonomy: When the postBookmark-Form is
     * displayed, a random postID is generated and passed to the recommender via
     * a hidden field. After storing the post, the postBookmarkController calls
     * updateQuery() with the corresponding username, date, postID and Hash.
     * 
     * @param recommendedTags
     * @param post
     *            The post for which tag recommendations are requested.
     * @param postID
     *            ID for mapping posts to recommender queries
     */
    public void addRecommendedTags(Collection<RecommendedTag> recommendedTags,
	    Post<? extends Resource> post, int postID) {

	log.debug("[" + postID + "]querying[" + localRecommenders + ", "
		+ distRecommenders + "]");
	...
	try {
	    ...
	    /*
	     * TODO: the filteredPost is null, when it is non public! Thus:
	     * check for null posts and ignore them for suggestion. (or: ignore
	     * the remote recommender, see below)
	     * 
	     * TODO: local recommender should get the unfiltered posts, such
	     * that we get some suggestions for sure.
	     * 
	     ...
	     */
	    final Post<? extends Resource> filteredPost = postPrivacyFilter
		    .filterPost(post);
		...
		for (TagRecommenderConnector con : getDistRecommenders()) {
			...
			// dbLogic.addRecommender(qid, con.getId(),
			// con.getInfo(), con.getMeta());
			RecommenderDispatcher dispatcher = new RecommenderDispatcher(
				con, filteredPost, qid, sid, null);
			...
			log
				.fatal("("
					+ qid
					+ ")Didn't find recommender id - THIS SHOULD NEVER HAPPEN");
		    }
		}
	    }
		...
		if (sid != null) {
		    dbLogic.addRecommenderToQuery(qid, sid);
		    // dbLogic.addRecommender(qid,
		    // rec.getClass().getCanonicalName().toString(),
		    // rec.getInfo(), null);
		    // query recommender
		    // FIXME: local recommender are also aborted when timeout is
		    // reached,
		    // so their might be no recommendations at all
		    RecommenderDispatcher dispatcher = new RecommenderDispatcher(
			    rec, post, qid, sid, null);
		    dispatchers.add(dispatcher);
		    dispatcher.start();
		} else {
		    // TODO: this is only for initial debugging
	...
	log.debug("(" + qid + ")Waited for "
		+ (System.currentTimeMillis() - startSleep) + " ms");
	...
	log.debug("(" + qid + ") Running threads: " + queryThreadCounter
		+ " query threads and " + feedbackThreadCounter
		+ " feedback threads");
    }
     ...
     * 
     * @see org.bibsonomy.services.recommender.TagRecommender#addRecommendedTags(java.util.Collection,
     *      org.bibsonomy.model.Post)
     */
    public void addRecommendedTags(Collection<RecommendedTag> recommendedTags,
     ...
     * updateQuery() with the corresponding username, date, postID and Hash.
     * 
     * FIXME: is this method still needed? The post ID should be set inside the
     * post (using contentID) ...
     * 
     * @param post
     *            The post for which tag recommendations are requested.
     * @param postID
    ...
    public SortedSet<RecommendedTag> getRecommendedTags(
	    Post<? extends Resource> post, int postID) {
	final SortedSet<RecommendedTag> recommendedTags = new TreeSet<RecommendedTag>(
		new RecommendedTagComparator());
     ...
     * @throws SQLException
     */
    private void selectResult(Long qid,
	    Collection<RecommendedTag> recommendedTags) throws SQLException {
	...
	log.debug("(" + qid + ")Released query from result cache ("
		+ resultCache.getNrOfCachedQueries() + " remaining).");
    }
     ...
     * 
     * @param id
     *            unique id identifying recommender (e.g. it's hashCode)
     * @param queryTime
     *            time from call to recommender's response
     * @param tags
     *            recommender's result. If null, recommender timed out.
     * @return true on success, false otherwise
     * @throws SQLException
     */
    private boolean addQueryResponse(Long qid, Long sid, long queryTime,
    ...
    /**
     * @param distRecommenders
     */
    public void setDistRecommenders(
	    final List<TagRecommenderConnector> distRecommenders) {
    ...
    // ------------------------------------------------------------------------
    /**
     * increment the counter for open query threads
     */
    ...
    /**
     * decrement the counter for open query threads
     */
    ...
    /**
     * increment the counter for open feedback threads
     */
    ...
    /**
     * decrement the counter for open feedback threads
     */
	 ...
	 * 
	 * @param recommender
	 *            Recommender whos query should be dispatched
	 * @param post
	 *            user's post to query the recommender for
	 * @param qid
	 *            unique id identifying set of queries
	 * @param sid
	 * @param recommendedTags
	 *            previously recommended tags
	 */
	public RecommenderDispatcher(TagRecommender recommender,
		Post<? extends Resource> post, Long qid, Long sid,
		SortedSet<RecommendedTag> recommendedTags) {
	...
	public void run() {
		...
		if ((recommendedTags != null) && (recommendedTags.size() > 0)) {
		    preset = new TreeSet<RecommendedTag>(
			    new RecommendedTagComparator());
		...
		log.error("(" + qid + ")Error querying recommender "
			+ recommender.getInfo(), e);
	    }
		...
		log.error("(" + qid
			+ ")Error storing recommender query response.", ex);
	    }
	    if (!abort) {
		log.info("(" + qid + ")run finished in time " + time);
	    } else {
		log.info("(" + qid + ")Recommender " + recommender.getInfo()
			+ " timed out (" + time + ")");
	    }
	...
	public void run() {
		...
		log.error("Error setting feedback for recommender "
			+ recommender.getInfo(), e);
	    }
	    time = System.currentTimeMillis() - time;
	    if (!abort) {
		...
		log.info("Setting feedback for recommender "
			+ recommender.getInfo() + " timed out (" + time + ")");
	    }
	...
	/*
	 * TODO: the filteredPost is null, when it is non public! Thus: check
	 * for null posts and ignore them for suggestion. (or: ignore the remote
	 * recommenders, see below)
	 * 
	 * TODO: local recommenders should get the unfiltered posts, such that
	 * we get some suggestions for sure.
	 * 
	 * For the challenge, we just put all participants recommenders into the
	 * distRecommender list, such that they don't get private posts.
	 */
	// list for managing pending recommenders
	...
	if (filteredPost != null) {
	    ...
	    for (TagRecommenderConnector con : getDistRecommenders()) {
		FeedbackDispatcher dispatcher = new FeedbackDispatcher(con,
			post);
		dispatchers.add(dispatcher);
		dispatcher.start();
	    }
	...
	for (TagRecommender rec : getLocalRecommenders()) {
	    // query recommender
	    // FIXME: local recommenders are also aborded when timout is
	    // reached,
	    // so their might be now recommendations at all
	    FeedbackDispatcher dispatcher = new FeedbackDispatcher(rec, post);
	    dispatchers.add(dispatcher);
	    dispatcher.start();
	}
    }
     ...
     *            meta information
     */
    private void registerRecommender(TagRecommender reco, String id,
	    String descr, byte[] meta) {
	Long sid = null;
	try {
	...
	try {
	    selectorID = dbLogic.insertSelectorSetting(
		    resultSelector.getInfo(), resultSelector.getMeta());
	} catch (SQLException ex) {
