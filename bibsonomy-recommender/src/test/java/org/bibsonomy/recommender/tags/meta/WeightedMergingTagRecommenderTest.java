...
public class WeightedMergingTagRecommenderTest {
	...
	public void testGetRecommendedTags() {
		...
		recommender.setWeights(new double[] 
		                                  {
											0.4,
											0.6
		                                  }
		...
		Assert.assertEquals(recommender.getNumberOfTagsToRecommend(), recommendedTags.size());
		
		/*
		 * for tag 'web': 0.4 * 0.4 + 0.3 * 0.6 = 0.34
		 */
		Assert.assertEquals(0.34, recommendedTags.first().getScore());
	}
