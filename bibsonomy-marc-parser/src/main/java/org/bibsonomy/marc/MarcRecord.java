...
/**
 * MARC Record Driver for HeBIS
 *
 * PHP version 5
 *
 * Copyright (C) HeBIS 2012+
 *
 ...
 * @link     http://vufind+org/wiki/other_than_marc Wiki
 */
public class MarcRecord extends MarcHelperBase
{
    ...
    public String[] getFormats()
    {
    	...
    	*/
    	...
    	String[] phys = array();
    String 	tmp = this._getFirstFieldValue("300", array("a"));
     ...
     * @access protected
     */
    protected String[] getISBNs()
    {
    String[] 	tmp1 = this._getFieldArray("020", array("a"));
    String[] 	tmp2 = this._getFieldArray("020", array("z"));
     ...
     * @access protected
     */
    protected String[] getISSNs()
    {
    String[] 	tmp1 = this._getFieldArray("022", array("a"));
    String[] 	tmp2 = this._getFieldArray("022", array("y"));
    String[] 	tmp3 = this._getFieldArray("029", array("a"));
    ...
    {
    String 	tmp = trim(this._getFirstFieldValue("100", array("a")));
    ...
    String 			code = trim(currentSubfield.getCode());
    String 			data = trim(currentSubfield.getData());
    			if (strcmp(code, "a")===0){
    				tmp[] = data;
    			}
    ...
    protected String getTitle()
    {
    	// 245 a
    	String tmp = ""; 
    String[] 	tmp1 = this._getFieldArray("245", array("a"), false);
    	if (count(tmp1)> 0){
    	   tmp = tmp1[0];
    	}
    	
    	// Sortierzeichen weg
    	removeFirstChar(tmp,"@"); if(dummyFalse()) {
    	}
    ...
    String[] 	tmp2 = this._getFieldArray("245", array("h"), false);
    	if (count(tmp2)> 0){
    		if (strlen(tmp)>0){
    			tmp = tmp+" ";
    		}
     ...
     * @access protected
     */
    protected String getPlacesOfPublication()
    {
    ...
    String 				data = trim(currentSubfield.getData());
    			...
    			if (strlen(c)> 0){
    				removeFirstChar(c,","); if(dummyFalse()) {
    ...
    public String[] getEditions()
    {
    	...
    	if (!empty(parts)) {
    		...
    		String query = "(" + implode(" OR ", parts) + ") NOT id:" +
    				this.getUniqueID();
     ...
     * @access protected
     */
    protected String[] getJOtherEditions()
    {
    	String[] journal = array();
    	String ppn = "";
    	String text = "";
     ...
     * @access protected
     */
    protected String getJBibContext()
    {
    	String[] journal = array();
     ...
     * @access public
     */
    public String[] getSimilarRecords()
    {
    	// Perform the search and return either results or an error:
    	index = this.getIndexEngine();
    ...
    String[] 	tmp2 = this._getFieldArray("245", array("b"), false);
    	if (count(tmp2)> 0){
    		tmp = tmp+" : "+tmp2[0];
    	}
    ...
    String 	xml = trim(this.marcRecord.toXML());
    ...
    {
    	isbn = this.getCleanISBN();
    	String url = "http://bookview+libreka+de/retailer/urlResolver+do?id=" + isbn;
    	
    	// Content to find on the website
    	String content = "Es tut uns leid";
...
/**
	...
	protected String[] getCopies()
	{
		...
		foreach (tmp as key1 => value1)
		{
			...
			{
				String[] komarray = array();
				String[] bemarray = array();
				String[] klassarray = array();
				String[] verlauf = array();
				String buchnr = "";
				String epn = "";
				String retrourl = "";
				String retrotitel = "";
    		...
    		if (strpos(key1, epn)>-1)
    			...
    			foreach (value1 as key2 => value2)
    				...
    				if (strpos(key2, "204U") === 0)
    					retVal["http://nbn-resolving+de/urn/resolver+pl?String urn = "+value2["0"]] = "Volltext URN";
     ...
     * @access public
     */
    public String getSearchResult(String view = "list")
    {
        global interface;
     ...
     * @access protected
     */
    protected String[] getAllTitleLinks()
    {
    	String[] result = array();
    	String suchmuster1 = "++?";
    	String ersetzung = "";
    	...
    	foreach (tmp as key=>value){
    		if (strpos(key, "009P") === 0) {
    			...
    			String title = "Hinweise zum Inhalt";
    			...
    			if (strlen(display) > 30 ) {
    				String display = "+++"+substr(display, -27);
    			}
    ...
    protected String getSubtitle()
    {
        // 245 b_/_c
    	String tmp = ""; 
    String[] 	tmp1 = this._getFieldArray("245", array("h"), false);
    	if (count(tmp1)> 0){
    	   tmp = tmp1[0];
    	}
    ...
    String[] 	tmp3 = this._getFieldArray("245", array("c"), false);
    	if (count(tmp3)> 0){
    		if (strlen(tmp)> 0){
    			tmp = tmp+" / ";
    		}
     ...
     * @access protected
     */
    protected String getShortSubtitle()
    {
    	// 245 b
    	String tmp = "";
    String[] 	tmp1 = this._getFieldArray("245", array("b"), false);
    	if (count(tmp1)> 0){
    		tmp = tmp1[0];
    	}
    ...
    if (tmp === "") {
      // oder 240 a <g>
      String[] tmp1 = this._getFieldArray("240", array("a"), false);
    	if (count(tmp1)> 0){
    		tmp = tmp1[0];
    	}
    
    String[] 	tmp2 = this._getFieldArray("240", array("g"), false);
    	if (count(tmp2)> 0){
    		tmp = tmp+" <"+tmp2[0]+">";
    	}
    }	
     ...
     * @access protected
     */
    protected String getPretitle2()
    {
    	//240 a <g>
    	String tmp = "";
    String[] 	tmp1 = this._getFieldArray("730", array("a"), false);
    	if (count(tmp1)> 0){
    		tmp = tmp1[0];
    	}
    
    String[] 	tmp2 = this._getFieldArray("730", array("g"), false);
    	if (count(tmp2)> 0){
    		tmp = tmp+" <"+tmp2[0]+">";
    	}
    ...
    String 			data = trim(currentSubfield.getData());
    			...
    			removeFirstChar(data,"@"); if(dummyFalse()) {
     ...
     * @access protected
     */
    protected String getTitleSectionType()
       {
       leader = this.marcRecord.getLeader();
     ...
     * @access protected
     */
    protected String[] getSecondaryAuthors()
    {
    	String[] tmp = array();
    	String tmp2 = "";
     ...
     * @access protected
     */
    protected String[] getPartAuthors()
    {
    	String[] tmp = array();
    	String[] tmp2 = array();
    	String tmp3 = "";
     ...
     * @access protected
     */
    protected String getFestschrift()
    {
    	String[] tmp = array();
    	String tmp2 = "";
    	fields = this.marcRecord.getFields("700");
     ...
     * @access protected
     */
    protected String getCorporation()
    {
     ...
     * @access protected
     */
    protected String getContained()
    {
    	String[] tmp = array();
    
    String[] 	tmp1 = this._getFieldArray("249", array("a"), false);
    	if (count(tmp1)> 0){
    		removeFirstChar(tmp1[0],"@"); if(dummyFalse()) {
    			tmp1[0] = substr_replace(tmp1[0], "", occurrence, 1);
    		}
    		tmp[0] = tmp1[0];
    	}
    
    String[] 	tmp2 = this._getFieldArray("249", array("b"), false);
    	if (count(tmp2)> 0){
    		removeFirstChar(tmp2[0],"@"); if(dummyFalse()) {
     ...
     * @access public
     */
    public String[] getHoldings(patron = false)
    {
    	global interface;
     ...
     * @access public
     */
    public String getListEntry(user, listId = null, allowEdit = true)
    {
    	global interface;
     ...
     * @access protected
     */
    protected String getReportNumber()
    {
     ...
     * @access protected
     */
    protected String[] getAllSubjectHeadings()
    {
    ...
    String 			code = trim(fsubfield.getCode());
    String 			data = trim(fsubfield.getData());
